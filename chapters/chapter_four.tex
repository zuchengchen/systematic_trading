...
\begin{figure}[htbp] 
	\centering \includegraphics[width=\textwidth]{figures/raw/img-015.png} \caption{HARDER TO DISTINGUISH SHARPE RATIOS THAN YOU THINK} \end{figure}
...
{\setlength{\fboxsep}{7pt}\setlength{\arrayrulewidth}{0.5pt}\fcolorbox{bookrule}{white}{%
\begin{tabular}{@{} >{\raggedright\arraybackslash}p{0.5\textwidth} c >{\raggedright\arraybackslash}p{0.3\textwidth} @{}}
...
\medskip \noindent\textbf{First level grouping, by equity industry within country and by bond country.} Within equities I am going to assume stocks have similar correlations if they are within the same industry and country.
\begin{list}{$\bullet$}{%
...
\item UK banks: assuming similar correlations allocate 33.3\% to each. Row 3.
\item UK retail: two assets so allocate 50\% to each. Row 2.
\item US banks: similar correlations so allocate 33.3\%. Row 3.
\item US retail: similar correlations so allocate 33.3\%. Row 3.
\item UK bonds: two assets so allocate 50\% to each. Row 2.
\item US bonds: for the three US bonds things are a little more complex. From table 55 (page 294) the 2 year and 20 year bonds typically have 0.5 correlation, 2 year/30 year 0.5 and 20 year/30 year 0.9. This matches row 10 of table 8, giving weights of 42\% in the 2 year bond and 29\% in each of the 20 and 30 year bonds.
\end{list}

\medskip \noindent\textbf{Second level grouping, by country within asset class.}
\begin{list}{$\bullet$}{%
  \setlength{\leftmargin}{1.5em}%
  \setlength{\itemsep}{0pt}%
  \setlength{\parsep}{0pt}%
...
\item UK equities: two groups (UK banks and UK retailers) so allocate 50\% to each. Row 2.
\item US equities: two groups (US banks and US retailers) so allocate 50\% to each. Row 2.
\item UK bonds: 100\% as only one group. Row 1.
\item US bonds: 100\%, one group. Row 1.
\end{list}
...
\subsection*{Follow these steps to adjust handcrafted weights for Sharpe ratio}

\noindent{\small \setlength{\tabcolsep}{3pt} \renewcommand{\arraystretch}{1.12} 
...
\medskip Now for the second level where I mix bonds and equities
...
\subsection*{Once again, are we cheating?}

Now you’re using \textbf{Sharpe ratios (SR)} to produce your handcrafted weights it’s worth reiterating that this is a mild form of \textbf{in-sample back-test} cheating, since you only use the final SR averaged over all data history, which you wouldn’t have at the beginning of the back-test.\footnote{As with the standard handcrafted weights it's possible to back-test this by doing the SR adjustment on an \textbf{expanding window}. For the first ten years of data you shouldn't adjust the weights at all. After that you should use only past data to estimate the SR at each point in the back-test. But again this is much more work than using the \textbf{bootstrapping} method.}
...
\begin{tabular}{@{} >{\raggedright\arraybackslash}p{0.64\textwidth} >{\centering\arraybackslash}p{0.2\textwidth}@{}}
\toprule
