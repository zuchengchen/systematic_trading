...
\section*{The A and B system: Early profit taker and early loss taker} \phantomsection \addcontentsline{toc}{section}{The A and B system: Early profit taker and early loss taker}
...
Notice that this is a complete \textbf{trading system}, not just a \textbf{trading rule}, since it tells you exactly what size positions to take. If you've read part three of the book you can probably see that the system does position sizing correctly, but for an arbitrary \textbf{volatility target} on a single \textbf{instrument}. Here are some interesting characteristics of the A and B system.
...
The stop loss rule defined for \textbf{semi-automatic traders} in part four of the book is based on the stop loss component of the A and B system.

\section*{The exponentially weighted moving average crossover (EWMAC) rule} \phantomsection \addcontentsline{toc}{section}{The exponentially weighted moving average crossover (EWMAC) rule}

The \textbf{EWMAC} trading rule was briefly introduced in chapter seven. The forecast is the difference, or crossover, between two \textbf{exponentially weighted moving averages} (EWMA) of the price, one fast and one slow. The difference is then standardised by the recent \textbf{standard deviation} of prices, and a \textbf{forecast scalar} applied to ensure we get an average absolute value of 10 for our forecasts.
...
\section*{The carry trading rule} \phantomsection \addcontentsline{toc}{section}{The carry trading rule}

I introduced the \textbf{carry} rule in chapter seven. The rule calculates an annualised \textbf{volatility} standardised expected return for the asset, assuming nothing happens to prices. Because the carry rule is slightly different for various \textbf{asset classes}, the definition here is in two parts. The first part calculates the annualised net expected return in price points for different assets. Then in the second part, which applies to all assets, this is converted to a forecast.
...
The raw carry measure is effectively an annualised \textbf{Sharpe ratio (SR)}, an expected return divided by standard deviation. I used the technique in appendix D and data from a large number of markets across different \textbf{asset classes} to work out the right forecast scalar. This gives a forecast scalar of around 30.

\subsection*{What is the turnover of carry?}

It's hard to generalise about the \textbf{turnover} (round trips per year) of carry since it depends on the asset class and how often you update the value of the forecast. I suggest checking the forecast weekly to avoid spurious noise which can otherwise be a problem. If you do this then it is reasonable to use a rule of thumb value of 10 for the turnover of the carry rule.
