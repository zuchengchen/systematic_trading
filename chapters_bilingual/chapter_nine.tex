...
\bisubsectiontitle{假设 Sharpe 比率为 0.50，偏度为零。103}
...
在一项长达 35 年、采用保守的样本外 bootstrapping 拟合的回测中，它在扣除成本后的夏普比率
...
一个不错的经验法则是使用表 14
（第 90 页）中的比例。它们表明：如果你像我一样采用样本外 bootstrap，那么应当乘以 0.75，才能得到更现实的夏普比率。
...
因此，采用 Half-Kelly，把风险设定为最优值的一半，会好得多。表 25 的 A 列给出了：在某个现实可行的回测 SR 下，我建议采用的百分比波动率目标。
以我自己的系统为例，我先从 1.0 的回测夏普比率出发，再根据表 14 乘以 0.75
（因为我使用的是样本外 bootstrapping），得到更现实的 SR = 0.75。
...
不同收益偏度
（列）与可实现的回测夏普比率
（行）对应的建议百分比波动率目标。最优波动率目标是用 Half-Kelly 计算出来的。
