...
\textbf{纳入 Sharpe 比率} & 如何利用关于预期表现的额外信息来改进手工权重。 \\
\end{tabularx}
...
手工构造法是一种自下而上的建仓流程。你先把相似资产分成小组，在组内根据几种典型相关性结构分配权重；然后再给“组的组合”分配权重，层层向上，直到得到整个投资组合。这个方法默认资产的预期波动率大致相当，且在第一步里也暂时假设它们的 Sharpe 比率相近。真正重要的是相关性的大致结构，而不是伪精确的数字。
...
\bisectiontitle{把 Sharpe 比率纳入权重}
...
但有时你会拥有一个更可信的“相对 Sharpe 看法”。最常见的来源之一，就是成本：不同资产的成本差异通常比“谁的原始表现更好”更容易被可靠估计。
...
\rawtablecaption{HOW MUCH SHOULD YOU ADJUST HANDCRAFTED WEIGHTS BY IF YOU HAVE SOME INFORMATION ABOUT ASSET SHARPE RATIOS?\\如果你掌握了关于资产 Sharpe 比率的一些信息，应当如何调整手工权重？}
\begingroup
\small
\centering
\resizebox{\textwidth}{!}{%
...
请按照下面这些步骤，用 Sharpe 比率来调整手工构造权重：

起始权重 先求出该组的手工构造起始权重。它们应当加总为 100\%。

计算 Sharpe 比率 在每个组内，利用历史数据、成本估计或其他方法，
求出每项资产的预期 SR。
...
\bisubsectiontitle{Sharpe 相对平均值：平均为 0.25，偏离如下}
...
如果 NASDAQ 的估计 Sharpe 比率是 0，而 S\&P 500 是 0.5，那么相对于组内平均值 0.25，NASDAQ 就是 -0.25，S\&P 500 则是 +0.25。
...
\rawtablecaption{HOW MUCH OF AN EFFECT DOES INCLUDING SHARPE RATIOS (SR) HAVE ON OPTIMISED PORTFOLIO WEIGHTS?\\把 Sharpe 比率纳入后，对组合权重会有多大影响？}
\begingroup
\small
\centering
\begin{tabular}{@{}lcccc@{}}
...
当我把 Sharpe 比率估计纳入后，handcrafting 会提高表现更好资产的权重，
