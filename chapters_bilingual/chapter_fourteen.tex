...
采用 20 周回看窗口时，你的交易换手率大约会是每年 0.4 次往返交易。如果你采用第十二章中我建议的速度上限，
也就是每年在成本上不超过 0.08 个 SR，那么每年 0.4 次往返交易就意味着你所交易的任何工具，其最大标准化成本必须不高于
...
如果你进一步假设这个波动率目标正好位于正确的半 Kelly 水平，那么所要求的夏普比率
...
（比如债券）的权重
（甚至彻底剔除它们）来提高夏普比率。这一结构下的成本会是多少呢？前面我算过：你的换手率大约是每年 0.4 次往返交易，
...
（第 86 页）B 列 “Without certainty” 来进行夏普比率权重调整。 \\
\textbf{投资组合品种仓位} & 子系统仓位乘以品种权重
（本例使用表 41 的权重，如有需要再做夏普比率调整），并再乘以品种分散化乘数
（本例中为 1.61）。 \\
