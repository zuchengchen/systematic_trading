...
\textit{这张表展示的是：当你从不同规模的“原本并不盈利”的规则池中筛选规则时，
在给定不同最低夏普门槛的情况下，平均会有多少条规则被误选出来。}
...
连那些真正不错的规则也会被误杀；
毕竟，
一条真实交易规则在单一品种上的现实 SR，
大概率也不过只有 0.30 左右。
那我们就假设：
我愿意接受“至少错选出一条真正不盈利规则”的概率不超过 5\%。
另外，
真实世界里我通常也不只拥有一年的数据，
所以接下来我们就来看看：
历史长度增加之后，
...
收益序列的长度在这里非常关键。
历史越长，
一个真实 SR 为零的规则就越不容易仅仅靠运气走出一段漂亮表现，
于是我就可以把筛选门槛适当放低。
但即便拥有 30 年数据，
我仍然不敢测试太多规则；
而且即便如此，
当许多完全不错的变体真实 SR 也不过只有 0.3 时，
这个门槛依然会高得离谱。
...
\textit{这张表展示的是：当你要在不同规模的规则池中筛选规则时，
为了把“误选出至少一条真正坏规则”的概率控制在 5\% 以内，
所需的夏普比率门槛会随着历史样本长度如何变化。}
...
这个检验通常会采用“两倍 sigma”的门槛。
如果一条规则的估计平均 SR 高于零两个标准差以上，
那么在真实 SR 实际上为负的情况下，
出现这种结果的概率只有 2.5\%。
...
\textit{图 11 展示了一条任意规则的平均测得 SR 如何随时间演化，
以及围绕它的上下“置信区间”。
每个置信区间都与平均值相差两个 sigma；
因此，当下界也被推到零以上时，
这条系统其实是亏钱货色的概率就只剩下 2.5\%。}
...
\textit{如果我把同样实验重复在一条真实 SR 为 1.0 的规则上，
得到的就是图 12。
这一次通过检验只需要几年时间。
平均而言，
真实 SR 越高的策略，
...
而且往往还远不止十年，
才能有把握说这条策略真的能赚钱。
而一条单一品种交易规则现实中比较合理的 SR 大约只有 0.3，
这意味着你几乎需要 40 年历史。}
...
需要 45 年的数据。
这几乎是不可能的，
因为最早的金融期货也不过从 1972 年才开始交易。
但如果同样的规则运行在一个品种组合上，
它们的 SR 就会变成 0.13 和 1.13。
这时表格显示，
我只需要 11 年数据，
就足以把这两个夏普比率区分开来。
