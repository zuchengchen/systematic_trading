...
\noindent\textsc{一旦你决定运行一套系统化交易系统，你就必须从若干交易规则中做出选择，并把其他规则淘汰掉。}
...
老板兴奋地介绍：“这是 Joe。他才来一个月，就已经搞出了 50 条在回测里赚钱的新交易规则！”
Joe 也补充道：“对，这软件太神了。它一天能自动测试几百条规则。”
...
拟合里最常见的第一个错误，就是假装自己拥有时光机。

任何拟合过程里，始终存在两个不同时间段：
一个是用来拟合模型的时期，
另一个是用来测试模型的时期。
比如，假设你有十年的 GBP/USD 日度价格数据，
并且想在若干变体中挑出“最好”的那一个。

最常见也最偷懒的方法是：
用整整十年的全部数据来找到过去十年中最赚钱的那一个变体，
然后再回头看它在同样这十年里的每一年表现如何。
这当然会得到漂亮得不像真的回测结果。
问题在于：
1990 年的你，根本不可能提前知道 2000 年的数据，
也就不可能在 1990 年真的做出那样的规则选择。
这就是典型的样本内回测
（in-sample back-test）：
同一批数据既被用来选择规则，
又被用来证明规则有效。
...
问题是，
你会白白浪费掉一半数据，
而且也无法反映后半段里市场结构可能发生的变化。

我最偏好的方法是扩张式样本外测试
（expanding window）。
它的逻辑是：
在每一年做测试时，
只使用那一年之前所有已经出现的数据来拟合。
这样既没有作弊，
又尽可能充分地使用了过去。
它的缺点是：
如果世界已经变了，
你仍然会把那些可能早已失效的历史数据继续算进去。
为了解决这一点，
你也可以使用滚动窗口
（rolling window 或 walk-forward）：
永远只用最近五年或十年的数据来拟合，
更早的数据就丢掉。
...
想法越少，
过拟合的机会就越低，
而在接受一条规则前所需要达到的临界 Sharpe 比率也会更低
（见表 4）。
2. 同一种市场行为有很多种捕捉方式：
例如，
